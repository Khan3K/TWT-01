...
if (isset($_POST['save_customer'])) {
    ...
    try {
        ...
        log_activity($id ? 'UPDATE' : 'CREATE', 'customers', $log_id, ($id ? 'Updated' : 'Added') . " customer: $name");
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        redirect('customers.php', "Cannot delete customer: " . $e->getMessage(), 'danger');
    }
}
                            ...
                            <td>
                                <div class="d-flex align-items-center" style="gap: 10px;">
                                    <div style="width: 36px; height: 36px; border-radius: 10px; background: linear-gradient(135deg, #e0e7ff, #c7d2fe); display: flex; align-items: center; justify-content: center; color: #4f46e5; font-weight: 600; font-size: 0.75rem;">
                                        <?php echo strtoupper(substr($cus['name'], 0, 1)); ?>
                                    </div>
                                    <span class="fw-bold"><?php echo $cus['name']; ?></span>
                                </div>
                            </td>
                            <td><i class="fas fa-phone me-1 text-muted" style="font-size: 0.75rem;"></i><?php echo $cus['phone']; ?></td>
                            <td><i class="fas fa-envelope me-1 text-muted" style="font-size: 0.75rem;"></i><?php echo $cus['email']; ?></td>
                            <td class="text-muted"><?php echo $cus['address']; ?></td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-info edit-cus-btn" data-json='<?php echo json_encode($cus); ?>' data-bs-toggle="modal" data-bs-target="#customerModal" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </button>
                                <a href="customers.php?delete=<?php echo $cus['customer_id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')" title="Delete">
