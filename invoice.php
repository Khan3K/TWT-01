...
$sale = $conn
    ->query(
        "SELECT s.*, c.name as customer_name, c.phone as customer_phone, c.email as customer_email, c.address as customer_address, u.full_name as cashier_name FROM sales s LEFT JOIN customers c ON s.customer_id = c.customer_id LEFT JOIN users u ON s.user_id = u.user_id WHERE s.sale_id = $sale_id",
    )
    ->fetch_assoc();
...
$items = $conn->query(
    "SELECT si.*, m.medicine_name, m.batch_no, m.expiry_date FROM sale_items si LEFT JOIN medicines m ON si.medicine_id = m.medicine_id WHERE si.sale_id = $sale_id",
);
...
<div class="wrap">
        ...
        <div class="inv-hdr">
            <div class="hdr-top">
                ...
                <div class="inv-badge">
                    ...
                    <div class="inv-date"><?php echo format_date(
                        $sale["sale_date"],
                        "d M Y",
                    ); ?> at <?php echo format_date(
     $sale["sale_date"],
     "h:i A",
 ); ?></div>
                </div>
        ...
        <div class="inv-body">
            ...
            <div class="info-row">
                <div class="info-box">
                    <h5>👤 Bill To</h5>
                    <p class="name"><?php echo $sale["customer_name"] ?:
                        "Walk-in Customer"; ?></p>
                    <p>
                        <?php if (
                            $sale["customer_phone"]
    ...
    "customer_email"
                        ...
                        ): ?>📍 <?php echo $sale["customer_address"];endif; ?>
                        <?php if (
                            !$sale["customer_phone"] &&
                            !$sale["customer_email"] &&
                            !$sale["customer_address"]
                ...
                <div class="info-box">
                    <h5>💳 Payment</h5>
                    <p>
                        <strong>Method:</strong> <?php echo ucfirst(
                            $sale["payment_method"],
                        ); ?><br>
                        <strong>Status:</strong> <span style="color:<?php echo $sale[
                            "payment_status"
                        ] == "paid"
                            ? "#059669"
                            : "#f59e0b"; ?>;font-weight:700;"><?php echo ucfirst(
    $sale["payment_status"],
); ?></span><br>
                        <strong>Served by:</strong> <?php echo $sale[
                            "cashier_name"
                        ]; ?>
                    </p>
                </div>
                <div class="info-box">
                    <h5>📋 Summary</h5>
                    <p>
                        <strong>Date:</strong> <?php echo format_date(
                            $sale["sale_date"],
                            "d M Y",
                        ); ?><br>
                        <strong>Time:</strong> <?php echo format_date(
                            $sale["sale_date"],
                            "h:i A",
                        ); ?><br>
            ...
            <table class="inv-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo $i++; ?></td>
                        <td class="med-name"><?php echo $item[
                            "medicine_name"
                        ]; ?></td>
                        <td><span class="batch"><?php echo $item["batch_no"] ??
                            "N/A"; ?></span></td>
                        <td><span class="exp"><?php echo $item["expiry_date"]
                            ? format_date($item["expiry_date"], "M Y")
                            : "N/A"; ?></span></td>
                        <td class="c"><?php echo $item["quantity"]; ?></td>
                        <td class="r"><?php echo format_currency(
                            $item["unit_price"],
                        ); ?></td>
                        <td class="r" style="font-weight:600;"><?php echo format_currency(
                            $item["subtotal"],
                        ); ?></td>
                    </tr>
            ...
            <div class="totals-wrap">
                <div class="totals">
                    <div class="tot-row">
                        ...
                        <span class="val"><?php echo format_currency(
                            $sub,
                        ); ?></span>
                    </div>
            ...
            <!-- BADGES -->
            <div class="badges">
                <span class="badge badge-<?php echo $sale[
                    "payment_method"
                ]; ?>">
                    <?php echo $sale["payment_method"] == "cash"
                        ? "💵"
...
// Auto actions from query params
