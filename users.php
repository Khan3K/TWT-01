...
if (isset($_POST['save_user'])) {
    ...
    if ($id) {
        if (!empty($password)) {
            ...
            $stmt = $conn->prepare("UPDATE users SET username=?, full_name=?, role=?, password=? WHERE user_id=?");
            $stmt->bind_param("ssssi", $username, $full_name, $role, $hashed_pass, $id);
        } else {
        ...
        if (empty($password)) {
            redirect('users.php', "Password is required for new users!", 'danger');
        }
        $hashed_pass = password_hash($password, PASSWORD_DEFAULT);
    ...
    try {
        ...
        log_activity($id ? 'UPDATE' : 'CREATE', 'users', $log_id, ($id ? 'Updated' : 'Added') . " user: $full_name ($username)");
        redirect('users.php', $msg);
    } catch (Exception $e) {
        $errMsg = $e->getMessage();
        if (strpos($errMsg, 'Duplicate') !== false || strpos($errMsg, '1062') !== false) {
...
if (isset($_GET['delete'])) {
    ...
    if ($id != $_SESSION['user_id']) {
        ...
        try {
            ...
            redirect('users.php', "Cannot delete user: " . $e->getMessage(), 'danger');
        }
    } else {
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center" style="gap: 10px;">
                                    <div style="width: 36px; height: 36px; border-radius: 10px; background: linear-gradient(135deg, <?php echo ($u['role'] == 'Admin') ? '#818cf8, #6366f1' : (($u['role'] == 'Manager') ? '#34d399, #10b981' : '#38bdf8, #0ea5e9'); ?>); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 600; font-size: 0.75rem;">
                                        <?php echo strtoupper(substr($u['full_name'], 0, 1)); ?>
                                    </div>
                                    <span class="fw-bold"><?php echo $u['full_name']; ?></span>
                                </div>
                            </td>
                            <td class="text-muted"><?php echo $u['username']; ?></td>
                            <td>
                                <?php if($u['role'] == 'Admin'): ?>
                                    <span class="badge" style="background: rgba(99,102,241,0.1); color: #6366f1;">
                                        <i class="fas fa-shield-alt me-1" style="font-size: 0.7rem;"></i><?php echo $u['role']; ?>
                                    </span>
                                <?php elseif($u['role'] == 'Manager'): ?>
                                    <span class="badge" style="background: rgba(16,185,129,0.1); color: #059669;">
                                        <i class="fas fa-user-tie me-1" style="font-size: 0.7rem;"></i><?php echo $u['role']; ?>
                                    </span>
                                <?php else: ?>
                                    <span class="badge" style="background: rgba(14,165,233,0.1); color: #0ea5e9;">
                                        <i class="fas fa-user me-1" style="font-size: 0.7rem;"></i><?php echo $u['role']; ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-info edit-user-btn" data-json='<?php echo json_encode($u); ?>' data-bs-toggle="modal" data-bs-target="#userModal" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </button>
                                <?php if($u['user_id'] != $_SESSION['user_id']): ?>
                                <a href="users.php?delete=<?php echo $u['user_id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')" title="Delete">
