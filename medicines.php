...
check_login();
check_role(["admin", "pharmacist", "manager"]);
$can_manage_medicines = has_role(["admin"]);

// Handle Add/Edit
if (isset($_POST["save_medicine"])) {
    if (!$can_manage_medicines) {
        redirect(
            "medicines.php",
            "Access Denied! You cannot modify medicines.",
            "danger",
        );
    }
    $id = $_POST["id"];
    ...
    if ($id) {
        $stmt = $conn->prepare(
            "UPDATE medicines SET medicine_name=?, category_id=?, supplier_id=?, batch_no=?, expiry_date=?, purchase_price=?, selling_price=?, quantity=?, reorder_level=? WHERE medicine_id=?",
        );
        $stmt->bind_param(
            "siissddiii",
            $name,
        ...
        $stmt = $conn->prepare(
            "INSERT INTO medicines (medicine_name, category_id, supplier_id, batch_no, expiry_date, purchase_price, selling_price, quantity, reorder_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        );
        $stmt->bind_param(
            "siissddii",
            $name,
    ...
    try {
        ...
        log_activity(
            ...
            "medicines",
            $log_id,
            ($id ? "Updated" : "Added") . " medicine: $name",
        );
...
if (isset($_GET["delete"])) {
    if (!$can_manage_medicines) {
        redirect(
            "medicines.php",
            "Access Denied! You cannot delete medicines.",
            "danger",
        );
    }
    $id = (int) $_GET["delete"];
    ...
    try {
        ...
        redirect(
            "medicines.php",
            "Cannot delete medicine: " . $e->getMessage(),
            "danger",
        );
    }
}
...
$medicines = $conn->query(
    "SELECT m.*, c.category_name as cat_name, s.supplier_name as sup_name FROM medicines m LEFT JOIN categories c ON m.category_id = c.category_id LEFT JOIN suppliers s ON m.supplier_id = s.supplier_id $where ORDER BY m.medicine_id DESC",
);
...
<div class="page-header">
    ...
    <button class="btn btn-primary" id="addMedicineBtn" data-bs-toggle="modal" data-bs-target="#medicineModal">
...
$_GET["category"] == $cat["category_id"]
    ...
    : ""; ?>><?php echo $cat["category_name"]; ?></option>
            ...
            <div class="col-md-3 d-flex gap-2">
                ...
                <a href="medicines.php" class="btn btn-secondary" title="Clear filters">
                        ...
                        <tr class="<?php echo $rowClass; ?>">
                            <td>
                                <div class="d-flex align-items-center" style="gap: 12px;">
                                    <div style="width: 40px; height: 40px; border-radius: 10px; background: <?php echo $isExpired
                                        ? "rgba(239,68,68,0.1)"
                                        : ($isLow
                                            ? "rgba(245,158,11,0.1)"
                                            : "rgba(99,102,241,0.1)"); ?>; display: flex; align-items: center; justify-content: center;">
                                        <i class="fas fa-pills" style="color: <?php echo $isExpired
                                            ? "#ef4444"
                                            : ($isLow
                                                ? "#f59e0b"
                                                : "#6366f1"); ?>; font-size: 0.9rem;"></i>
                                    </div>
                                    <div>
                                        <div class="fw-bold" style="font-size: 0.9rem;"><?php echo $med[
                                            "medicine_name"
                                        ]; ?></div>
                                        <small class="text-muted">Batch: <?php echo $med[
                                            "batch_no"
                                        ]; ?></small>
                                    </div>
                                </div>
                            </td>
                            <td><span class="badge bg-light text-dark"><?php echo $med[
                                "cat_name"
                            ]; ?></span></td>
                            ...
                            <td>
                                <span class="fw-bold <?php echo $isLow
                                    ? "text-warning"
                                    : "text-success"; ?>">
                            ...
                            <td class="text-end fw-bold"><?php echo format_currency(
                                $med["selling_price"],
                            ); ?></td>
                            <td class="text-center">
                                <?php if ($can_manage_medicines): ?>
                                <button class="btn btn-sm btn-info edit-med-btn" data-json='<?php echo htmlspecialchars(
                                    json_encode($med),
                                    ENT_QUOTES,
                                    "UTF-8",
                                ); ?>' data-bs-toggle="modal" data-bs-target="#medicineModal" title="Edit medicine" aria-label="Edit medicine">
                                    <i class="fas fa-pen"></i>
                                </button>
                                <a href="medicines.php?delete=<?php echo $med[
                                    "medicine_id"
                                ]; ?>" class="btn btn-sm btn-danger" data-confirm="Delete this medicine permanently?" title="Delete medicine" aria-label="Delete medicine">
    ...
    "category_name"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="supplier_id" id="med_sup" class="form-select">
                                ...
                                <?php while (
                                    $sup = $suppliers->fetch_assoc()
                                ): ?>
                                    <option value="<?php echo $sup[
                                        "supplier_id"
                                    ]; ?>"><?php echo $sup[
    "supplier_name"
]; ?></option>
...
const medicineForm = medicineModal.querySelector('form');

medicineModal.addEventListener('show.bs.modal', (event) => {
    const trigger = event.relatedTarget;
    if (!trigger || !trigger.classList.contains('edit-med-btn')) {
        medicineForm.reset();
        document.getElementById('med_id').value = '';
        document.getElementById('med_reorder').value = '10';
        document.getElementById('med_qty').value = '0';
    }
});
...
document.querySelectorAll('.edit-med-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.getElementById('med_qty').value = med.quantity || '0';
    });
});
</script>
<?php endif; ?>
